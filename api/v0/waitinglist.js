...
router.get('/', async (req, res, next) => {
    ...
    try {
        ...
        if (zero_data.items.length == 0) {
            res.status(204).json({ status: 204, msg:"Waitinglist is null. Try again later."})
        }
        lateste_num_data.items[0].num++;
        const result_upd = await db.update(latest_num_data.items[0], latest_num_data.items.key);
